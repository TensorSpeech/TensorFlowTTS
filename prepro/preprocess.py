...
from prepro.experiment.example_dataset import LJSpeechProcessor
...
def preprocess():
    ...
    train_split, valid_split, _, _ = train_test_split(
        processor.items, [i[-1] for i in processor.items], test_size=config["test_size"], random_state=42, shuffle=True,
    )
...
def compute_statistics():
    ...
    for mel, f0, energy in tqdm(
        ...
        scaler_f0.partial_fit(f0.reshape(-1, 1))
